...
@interface GPAlertActionModel : NSObject
@property (nonatomic, copy) NSString * title;
@property (nonatomic, copy) UIColor *titleColor;
@property (nonatomic, assign) UIAlertActionStyle style;
@end
@implementation GPAlertActionModel
- (instancetype)init{
    if (self = [super init]) {
        self.title = @"";
...
typedef void (^GPAlertActionsConfig)(GPAlertActionBlock actionBlock);

@interface GPAlertController ()
//JXTAlertActionModel数组
@property (nonatomic, strong) NSMutableArray <GPAlertActionModel *>* alertActionArray;
//是否操作动画
@property (nonatomic, assign) BOOL setAlertAnimated;
//action配置
- (GPAlertActionsConfig)alertActionsConfig;
@end

@implementation GPAlertController
...
#pragma mark - Private
//action-title数组
- (NSMutableArray<GPAlertActionModel *> *)alertActionArray{
...
//action配置
- (GPAlertActionsConfig)alertActionsConfig{
    return ^(GPAlertActionBlock actionBlock) {
        if (self.alertActionArray.count > 0){
            //创建action
            __weak typeof(self)weakSelf = self;
            [self.alertActionArray enumerateObjectsUsingBlock:^(GPAlertActionModel *actionModel, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (GPAlertAction)addDefaultAction{
    ...
    return ^(NSString *title) {
        GPAlertActionModel *actionModel = [[GPAlertActionModel alloc] init];
...
+ (void)showAlertWithPreferredStyle:(UIAlertControllerStyle)preferredStyle title:(NSString *)title message:(NSString *)message appearanceProcess:(GPAlertAppearanceProcess)appearanceProcess actionsBlock:(GPAlertActionBlock)actionBlock{
    if (appearanceProcess){
        GPAlertController *alertMaker = [[GPAlertController alloc] initAlertControllerWithTitle:title message:message preferredStyle:preferredStyle];
        //防止nil
        if (!alertMaker) {
            return ;
        }
...
+ (void)showAlertWithTitle:(NSString *)title message:(NSString *)message appearanceProcess:(GPAlertAppearanceProcess)appearanceProcess actionsBlock:(GPAlertActionBlock)actionBlock{
...
+ (void)showActionSheetWithTitle:(NSString *)title message:(NSString *)message appearanceProcess:(GPAlertAppearanceProcess)appearanceProcess actionsBlock:(GPAlertActionBlock)actionBlock{
